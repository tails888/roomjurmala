...
function reviews(lang){
 ...
 return `<section class="section reviews-section" id="reviews" aria-labelledby="reviews-title"><div class="reviews-heading"><div><p class="reviews-eyebrow">${t.eyebrow}</p><h2 id="reviews-title">${t.title}</h2></div><a class="reviews-score" href="${esc(reviewSource)}" target="_blank" rel="noopener noreferrer"><strong>${lang==='en'?'5.0':'5,0'}<span>/ 5</span></strong><span>${stars}<span class="reviews-count">${t.count}</span></span>${diagonal}</a></div>
 <div class="reviews-grid">${customerReviews.map(r=>`<figure class="review-card"><span class="review-quote-mark" aria-hidden="true">“</span><blockquote cite="${esc(r.url)}" lang="lv"><p>${esc(r.quote)}</p></blockquote><figcaption><span class="review-initials" aria-hidden="true">${r.initials}</span><span><span class="review-author" lang="lv">${esc(r.author)}</span><a href="${esc(r.url)}" target="_blank" rel="noopener noreferrer" aria-label="${esc(t.original+' · '+r.author)}">${t.original}${diagonal}</a></span></figcaption></figure>`).join('')}</div><div class="reviews-bottom"><p>${t.note}</p><a class="text-link" href="${esc(reviewSource)}" target="_blank" rel="noopener noreferrer">${t.all}${diagonal}</a></div></section>`;
}
