...
import {buildRoom} from './tour-world.js?v=blender-1';
import {movePlayer,stops} from './tour-physics.js?v=blender-1';
...
export function mountTour(dialog,copy){
  ...
  let yaw=stops[0].yaw,pitch=stops[0].pitch,frame,previous=0,closed=false,ready=false,dirty=true,zone=0,transition;
  ...
  const paused=()=>!ready||!help.hidden||!photo.hidden||!error.hidden||document.hidden;
  ...
  stick.querySelectorAll('button').forEach(b=>on(b,'keydown',e=>{if(!paused()&&(e.key==='Enter'||e.key===' ')){e.preventDefault();const n={forward:[0,.5],back:[0,-.5],left:[-.5,0],right:[.5,0]}[b.dataset.dir];movePlayer(position,n[0]*Math.cos(yaw)-n[1]*Math.sin(yaw),-n[0]*Math.sin(yaw)-n[1]*Math.cos(yaw),world.obstacles);updateCamera();}}));
  ...
  function loop(now){
    ...
    // Embedded textures are ready with the model; redraw only when the view changes.
    if(ready&&dirty){renderer.render(scene,camera);dirty=false;}
    pointer.setAttribute('transform',`translate(${(position.x+5)*10},${(position.z+4.8)*10}) rotate(${-yaw*180/Math.PI})`);
  ...
  world.ready.then(()=>{
    if(closed)return;ready=true;dirty=true;loading.hidden=true;
    canvas.dataset.model='blender';if(!paused())canvas.focus({preventScroll:true});
  }).catch(e=>{
    if(closed)return;loading.hidden=true;error.hidden=false;dialog.classList.add('tour-unavailable');
    console.error('ROOM model unavailable',e);
  });
  on(window,'resize',resize);frame=requestAnimationFrame(loop);
  canvas.focus({preventScroll:true});
  return ()=>{
    closed=true;clear();unlock();abort.abort();cancelAnimationFrame(frame);clearTimeout(transition);
