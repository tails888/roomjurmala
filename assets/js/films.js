...
export function mountFilms(copy,reduced){
  ...
  function wanted(state){return (!state.video.hasAttribute('data-hero-video')||state.video.hasAttribute('data-media-ready'))&&!state.video.hasAttribute('data-scroll-scrub')&&state.visible&&active(state)&&!state.userPaused&&!document.hidden&&(!reduced.matches||state.manual);}
  function refresh(){
  ...
  document.addEventListener('room:media-ready',refresh);
